  ...
  render() {
        ...
        <Wrapper margin="0 0 70px 0">
          <Typography fontSize="2rem" color="gradient">
            {state['ready_' + this.user.uid]
              ...
              : 'Choice ships positions'}
          </Typography>
        </Wrapper>

        <Board
          array={
            state['ready_' + user.uid]
              ? state['data_' + user.uid].array
              : state.choice_data.array
          }
          show
        />
          ...
          <React.Fragment>
            ...
            <Wrapper>
              <Button
                ...
                onClick={
                  state['ready_' + this.user.uid] ? null : this.readyHandler
                }
              >
    ...
    return (
      <Container>
        ...
        {loading ? (
          <LinearProgress />
        ) : this.state['ready_' + user.uid] &&
          state['ready_' + state[this.user.uid]] ? (
          <GameRoomComponent />
        ) : (
          <ChoiceRoom />
        )}
